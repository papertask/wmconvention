		...
		function gdlr_get_blog_info( $array = array(), $wrapper = true, $sep = '' ){
			...
			if( !empty($theme_option['new-fontawesome']) && $theme_option['new-fontawesome'] == 'enable' ){
				$icon_class = array('date'=>'fa fa-clock-o', 'tag'=>'fa fa-tag', 'category'=>'fa fa-folder-o', 
					'comment'=>'fa fa-comment-o', 'author'=>'fa fa-user');
			}else{
			...
			foreach($array as $post_info){
				...
				switch( $post_info ){
					...
					case 'comment':
						...
						$ret .= '<a href="' . get_permalink() . '#respond" >' . get_comments_number() . ' ' . __('Comments', 'gdlr_translate') . '</a>';						
						$ret .= '</div>';						
						break;
					case 'author':
						ob_start();
						...
						$ret .= '<div class="blog-info blog-author">' . __('By', 'gdlr_translate') . ' ';
						$ret .= $author;
						$ret .= '</div>';						
						break;						
				}
			...
			if($wrapper && !empty($ret)){
				return '<div class="gdlr-blog-info gdlr-info gdlr-info-font">' . $ret . '<div class="clear"></div></div>';
			}else if( !empty($ret) ){
