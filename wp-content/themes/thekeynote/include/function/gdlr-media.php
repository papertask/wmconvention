	...
	*/	
	...
	if( !function_exists('gdlr_get_image') ){
		function gdlr_get_image($image, $size = 'full', $link = array(), $attr = ''){
			...
			if( !empty($link) ){
				$fancybox  = '<a href="' . esc_url($link['url']) . '" ';
		...
		function gdlr_get_image_frame_item($settings){
			...
			if( !empty($settings['link-type']) && $settings['link-type'] != 'none' ){
				...
				if( $settings['link-type'] == 'url' ){
					...
					$attribute = ' href="' . esc_url($settings['url']) . '" ';
					...
					$icon = gdlr_fa_class('icon-search');
					$attribute = ' href="' . esc_url($settings['url']) . '" data-rel="fancybox" ';
				}else if( $settings['link-type'] == 'video' ){
					$icon = gdlr_fa_class('icon-play');
					$attribute = ' href="' . esc_url($settings['url']) . '" data-rel="fancybox" data-fancybox-type="iframe" ';
				}
	...
	if( !function_exists('gdlr_get_gallery') ){
		function gdlr_get_gallery( $settings ){
			...
			if( (($paged - 1) * intval($settings['num-fetch'])) >= sizeof($settings['slider']) ){
				$paged = 1;
			}
