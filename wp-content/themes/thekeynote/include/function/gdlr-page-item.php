	...
	*/
	...
	if( !function_exists('gdlr_get_item_title') ){
		function gdlr_get_item_title( $atts ){
			...
			if( (!empty($atts['title-type']) && $atts['title-type'] != 'none') || !empty($atts['carousel']) ){
				...
				if(!empty($atts['title'])){
					$title_class = 'gdlr-title-' . (empty($atts['title-size'])? 'large': $atts['title-size']);
					$ret .= '<h3 class="gdlr-item-title gdlr-skin-title gdlr-skin-border ' . $title_class . '">' . $atts['title'] . '</h3>';
				}
				if(!empty($atts['caption']) && $atts['title-type'] == 'left-caption'){
					$ret .= '<div class="gdlr-item-title-caption gdlr-skin-info">' . $atts['caption'] . '</div>';
				}
				if( !empty($atts['carousel']) || (!empty($atts['right-text']) && !empty($atts['right-text-link'])) ){
					$ret .= '<div class="gdlr-item-title-nav">';
					if( !empty($atts['right-text']) && !empty($atts['right-text-link']) ){
						$ret .= '<a class="gdlr-item-title-link gdlr-info-font" href="' . esc_url($atts['right-text-link']) . '" >' . $atts['right-text'] . '</a>';
					}
	...
	if( !function_exists('gdlr_get_column_service_item') ){
		function gdlr_get_column_service_item( $settings ){
			...
			$ret .= '<div class="column-service-icon gdlr-skin-box"><i class="fa ' . $settings['icon'] . '" ></i></div>';
			...
			if( !empty($settings['learn-more-text']) && !empty($settings['learn-more-link']) ){
				$ret .= '<a class="column-service-learn-more gdlr-info-font" href="' . esc_attr($settings['learn-more-link']) . '" >' . $settings['learn-more-text'] . '</a>';
			}
	...
	if( !function_exists('gdlr_get_accommodation_item') ){
		function gdlr_get_accommodation_item( $settings ){
			...
			if( !empty($settings['button-text']) && !empty($settings['button-link']) ){
				$ret .= '<a class="accommodation-button-text gdlr-button" href="' . esc_attr($settings['button-link']) . '" >' . $settings['button-text'] . '</a>';
			}
	...
	if( !function_exists('gdlr_get_feature_media_item') ){
		function gdlr_get_feature_media_item( $settings ){
			...
			$ret .= '<div class="gdlr-item gdlr-feature-media-item gdlr-' . $settings['align'] . '" ' . $item_id . $margin_style . '>';
			...
			if( !empty($settings['button-link']) ){
				$ret .= '<a class="feature-media-button gdlr-button with-border" href="' . esc_url($settings['button-link']) . '" target="_blank">';
	...
	if( !function_exists('gdlr_get_skill_bar_item') ){
		function gdlr_get_skill_bar_item( $settings ){	
			...
			$ret .= '<div class="gdlr-skill-bar-progress" data-percent="' . esc_attr($settings['percent']) . '" ';
	...
	if( !function_exists('gdlr_get_pie_chart_item') ){
		function gdlr_get_pie_chart_item( $settings ){	
			...
			$ret .= '<div class="gdlr-chart gdlr-ux" data-percent="' . esc_attr($settings['progress']) . '" data-size="155" data-linewidth="8" ';
	...
	// subscribe item
	if( !function_exists('gdlr_get_subscribe_item') ){
		function gdlr_get_subscribe_item( $settings ){	
			$item_id = empty($settings['page-item-id'])? '': ' id="' . $settings['page-item-id'] . '" ';
			...
			$ret  = '<div class="gdlr-subscribe-item gdlr-item" ' . $item_id . $margin_style . '>';
			$ret .= '<h4 class="gdlr-subscribe-title">' . $settings['title'] . '</h4>';
			$ret .= '<div class="gdlr-newsletter-subscribe">'; 
			
			if( !class_exists('NewsletterSubscription') ){
				$ret .= esc_html__('Please install and activate the "Newsletter Plugin', 'thekeynote');
			}else{
				$ret .= NewsletterSubscription::instance()->get_form_javascript();
...
<?php		
	...
	// subscribe item
	if( !function_exists('gdlr_get_subscribe_item_old') ){
		function gdlr_get_subscribe_item_old( $settings ){	
			$item_id = empty($settings['page-item-id'])? '': ' id="' . $settings['page-item-id'] . '" ';
