<?php
	...
	*/
		...
		function gdlr_get_post_list( $post_type ){
			...
			if( !empty($post_list) ){
				foreach( $post_list as $post ){
					$ret[$post->post_name] = esc_html(str_replace('"', '', $post->post_title));
				}
			}
	...
	if( !function_exists('gdlr_convert_to_iso_date') ){	
		function gdlr_convert_to_iso_date($date, $time){
			preg_match('/\d\d?[\.:]\d\d?/', $time, $match);
			if( !empty($match[0]) ){
				$match = preg_split( '/[:\.\s]/', $time);

				$match[0] = substr('0' . $match[0], -2, 2);
				$match[1] = substr('0' . $match[1], -2, 2);
				
				$time = $match[0] . ':' . $match[1];
			}
